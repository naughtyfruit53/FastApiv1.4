...
const CompanySetupGuard: React.FC<CompanySetupGuardProps> = ({ children }) => {
  ...
  const { data: company, isLoading: companyLoading, refetch: refetchCompany } = useQuery({
    ...
    enabled: !!user && !user.is_super_admin && !isExemptRoute,
    retry: 1,
  });
  ...
  useEffect(() => {
    // Skip if user is not loaded, is super admin, or on exempt route
    if (!user || user.is_super_admin || isExemptRoute) {
      return;
    }
  ...
  }, [user, isCompanySetupRequired, isRestrictedRoute, hasShownToast, isExemptRoute]);
