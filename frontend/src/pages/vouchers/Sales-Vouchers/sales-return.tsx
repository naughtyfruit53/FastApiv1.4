// src/pages/vouchers/Sales-Vouchers/sales-return.tsx
...
const SalesReturnPage: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      let response;
      if (mode === 'create') {
        response = await api.post('/sales-returns', data);
        if (confirm('Voucher created successfully. Generate PDF?')) {
          handleGeneratePDF(response.data);
        }
      } else if (mode === 'edit') {
        response = await api.put('/sales-returns/' + data.id, data);
        ...
        <Table stickyHeader size="small">
          ...
          <TableBody>
            {latestVouchers.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} align="center">No sales returns available</TableCell>
              </TableRow>
            ) : (
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={12} md={4}>
              <TextField
                ...
                fullWidth
                placeholder="Voucher number, customer, notes..."
              />
            </Grid>
            <Grid item xs={6} md={3}>
              <TextField
