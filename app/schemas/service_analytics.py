...
class ReportConfigurationRequest(BaseModel):
    ...
    email_recipients: List[str] = Field(default_factory=list, description="Email recipients for scheduled reports")
...
class JobVolumeMetrics(BaseModel):
    ...
    peak_day: Optional[date] = Field(None, description="Day with highest job volume")
