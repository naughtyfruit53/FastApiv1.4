...
from app.models.base import User, Vendor, VendorFile
...
async def upload_vendor_file(
    ...
    existing_files_count = db.query(VendorFile).filter(
    ...
    try:
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
        logger.error(f"Error uploading vendor file: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_vendor_files(
    ...
    query = db.query(VendorFile).filter(
        VendorFile.vendor_id == vendor_id,
        VendorFile.organization_id == org_id
    )
    
    if file_type:
        query = query.filter(VendorFile.file_type == file_type)
...
async def download_vendor_file(
    ...
    # Get file record
    file_record = db.query(VendorFile).filter(
...
async def delete_vendor_file(
    ...
    try:
        ...
        logger.error(f"Error deleting vendor file: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
