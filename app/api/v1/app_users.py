...
async def create_app_user(
    ...
    try:
        ...
        try:
            supabase_user = supabase_auth_service.create_user(
                ...
                user_metadata={
                    ...
                    "organization_id": None,  # App-level users
            ...
            logger.info(f"Created app user {user_data.email} in Supabase Auth with UUID {supabase_uuid}")
            
        except SupabaseAuthError as e:
            logger.error(f"Failed to create app user {user_data.email} in Supabase Auth: {e}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Failed to create user in authentication system: {str(e)}"
            )
        except Exception as e:
            logger.error(f"Unexpected error creating app user {user_data.email} in Supabase Auth: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        try:
            ...
            logger.error(f"Failed to create app user {user_data.email} in local database: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
