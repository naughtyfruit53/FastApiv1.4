...
def log_license_creation(org_name: str, org_admin_email: str, created_by: str, success: bool = True, error: Optional[str] = None):
    """Log license creation events with comprehensive details"""
    logger = get_logger("license")
    status = "SUCCESS" if success else "FAILED"
    message = f"License Creation {status}: Organization '{org_name}' for admin {org_admin_email} by {created_by}"
    if error:
...
def log_password_change(user_email: str, change_type: str = "SELF", success: bool = True, error: Optional[str] = None, new_jwt_issued: bool = False):
    """Log password change events with JWT token information"""
    logger = get_logger("password")
    status = "SUCCESS" if success else "FAILED"
    message = f"Password Change {status}: {user_email} (Type: {change_type})"
    if new_jwt_issued:
        message += " - New JWT issued"
    if error:
        message += f" - Error: {error}"
    if success:
        logger.info(message)
